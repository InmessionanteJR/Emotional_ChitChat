...
class Transformer(nn.Module):
    ...
    def forward(self,  # type: ignore
        ...
        if merge_input is not None:
            ...
            # print(f'index:{index}')
            # print(f'top_k_index_indice:{top_k_index[1][0]}') # top_k_index[1][0].shape:torch.Size([64, 10])
            # print(f'top_k_index_value:{top_k_index[0][0]}')
            # print(f'index.shape:{index.shape}')
            # print(f'top_k_index[1].shape:{top_k_index[1].shape}')
            # print('-'*30)
            # print(f'index.shape:{index.shape}')    torch.Size([96])
            # print(f'V.shape:{V.shape}')
            # print(f'top_k_index[1][0].shape:{top_k_index[1][0].shape}')
            ...
            # weight=softmax(torch.reciprocal(top_k_index[0][0].max(dim=1)[0].reshape(-1, 1).repeat(1, top_k_index[0][0].shape[1]))*top_k_index[0][0],dim=1) # with temperature：the largest number in each row
            weight=softmax(50000*top_k_index[0][0],dim=1) # with temperature：the largest number in each row
            print(f'weight[0]:{weight[0]}')
            # print(f'weight:{weight}')
            weight_value_with_memory=torch.matmul(torch.unsqueeze(weight, dim=1),value_with_memory)


            prior_mean = weight_value_with_memory
            ...
            # print(f'posterior_log_variance_square.shape:{posterior_log_variance_square.shape}')
