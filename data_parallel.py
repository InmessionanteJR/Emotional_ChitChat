...
class MyDataParallel(torch.nn.DataParallel):
    ...
    **scattered** on dim specified (default 0). tuple, list and dict types will
    ...
    def forward(self, *inputs, **kwargs):
        #print("in forward of DataParallel")
        if not self.device_ids:
            return self.module(*inputs, **kwargs)

        for t in chain(self.module.parameters(), self.module.buffers()):
            #print("t.device:", t.device, "src_device_obj:", self.src_device_obj, "t", t)
            if t.device != self.src_device_obj:
                raise RuntimeError("module must have its parameters and buffers "
                                   "on device {} (device_ids[0]) but found one of "
                                   "them on device: {}".format(self.src_device_obj, t.device))

        #print("inputs before scatter:", inputs)
        #print("kwargs before scatter:", kwargs)
        #print("self.device_ids:", self.device_ids)
        inputs, kwargs = self.scatter(inputs, kwargs, self.device_ids)
        if len(self.device_ids) == 1:
            return self.module(*inputs[0], **kwargs[0])
        #print("self.device_ids[:len(inputs)]:", self.device_ids[:len(inputs)])
        #print("inputs:", inputs)
        #print("kwargs:", kwargs)
