...
class Transformer(nn.Module):
    ...
    def __init__(self, model_config, data_config, vocab: tx.data.Vocab):
        ...
        self.linear1 = torch.nn.Linear(self.config_model.hidden_dim,6) # 6: Affect-Driven Dialog Generation

        self.rnn = torch.nn.RNN(self.config_model.hidden_dim, self.config_model.hidden_dim,batch_first=True)

        self.linear2 = torch.nn.Linear(6,self.config_model.hidden_dim)
    ...
    def forward(self,  # type: ignore
        ...
        if merge_input is not None:
            new_input = merge_input.long()

            new_input = self.word_embedder(new_input)

            h_0 = torch.rand(1, encoder_input.size()[0], self.config_model.hidden_dim).to(device=new_input.device)
            _, c = self.rnn(new_input, h_0)
            c=self.linear1(c)
            c1=self.linear2(c)

            src_with_emotional=Variable(torch.rand(encoder_input.size()[0],list(encoder_input.size())[1]+1,self.config_model.hidden_dim)).to(device=src_input_embedding.device)
            src_with_emotional[:,0,:]=src_input_embedding[:,0,:]
            src_with_emotional[:,1,:]=c1

            for i in range(list(encoder_input.size())[1]-1):
                src_with_emotional[:,i+2,:]=src_input_embedding[:,i+1,:]
        ...
        if decoder_input is not None and labels is not None:
            ...
            # print('-' * 25)
            # print(f'outputs.logits:{torch.argmax(outputs.logits, dim=2) + 1}')
            # print('\n')
