...
class Transformer(nn.Module):
    ...
    def forward(self,  # type: ignore
        ...
        if merge_input is not None:
            ...
            similarity = torch.matmul(c, K.detach().t())
            # print(f'similarity:{similarity}')
            top_k_index=torch.topk(similarity,k=1000,dim=2) # [1] means 'indices'
            # print(f'index:{index}')
            ...
            weight=softmax(softmax(torch.log(top_k_index[0][0]),dim=1),dim=1)
