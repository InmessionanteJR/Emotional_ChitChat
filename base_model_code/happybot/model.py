...
class Transformer(nn.Module):
    ...
    def forward(self,
                ...
                linear_baseline:Optional[torch.nn.Module] = None,
                sentiment_classifier:Optional[torch.nn.Module] = None,
        ...
        # print(f'encoder_input.shape:{encoder_input.shape}') # encoder_input.shape:torch.Size([64, 81])
        ...
        if merge_input is not None:
            ...
            c1=condition_generator(new_input,self.word_embedder.embedding.get_device())
        ...
        if decoder_input is not None and labels is not None: # train-mode (teacher-forcing training)
            ...
            for i in range(encoder_input.shape[0]):
                src_sentence=str(tx.data.map_ids_to_strs(encoder_input[i].cpu(), vocab, strip_eos=None)) # str(<class 'numpy.str_'>) = <class 'str'>
                src_batch_sentence.append(src_sentence)
            res = tokenizer.batch_encode_plus(src_batch_sentence, pad_to_max_length=True)
            ...
            sentiment_score1 = softmax(Bert_outputs[0], dim=1)[:, 1]-0.5
            # print(f'sentiment_score.shape:{sentiment_score.shape}') # torch.Size([64, 81])



            # 2. system generated sentences' sentiment scores
            src_batch_sentence = []
            for i in range(outputs.logits.argmax(dim=2).shape[0]):
                src_sentence = str(tx.data.map_ids_to_strs(outputs.logits.argmax(dim=2)[i].cpu(), vocab,
                                                           strip_eos=None))  # str(<class 'numpy.str_'>) = <class 'str'>
                src_batch_sentence.append(src_sentence)
            res = tokenizer.batch_encode_plus(src_batch_sentence, pad_to_max_length=True)
            input_ids = torch.tensor(res['input_ids'], dtype=torch.long, device=self.device)
            # print(f'input_ids.shape:{input_ids.shape}')
            token_type_ids = res['token_type_ids']
            attention_mask = res['attention_mask']
            Bert_outputs = sentiment_classifier(input_ids=input_ids,
                                                attention_mask=torch.tensor(attention_mask, device=self.device),
                                                token_type_ids=torch.tensor(token_type_ids, device=self.device))
            sentiment_score2 = softmax(Bert_outputs[0], dim=1)[:, 1] - 0.5


            P_wt=softmax(outputs.logits, dim=2).max(dim=2)[0]


            # 3. users' second generated responses
            encoder_input=outputs.logits.argmax(dim=2)
            ...
            # print(f'predictions.shape:{predictions.shape}') # torch.Size([64, 74])

            pred_batch_sentence = []

            for i in range(predictions.shape[0]):
                pred_sentence = str(tx.data.map_ids_to_strs(predictions[i].cpu(), vocab,
                                                            strip_eos=None))  # str(<class 'numpy.str_'>) = <class 'str'>
            ...
            Bert_outputs = sentiment_classifier(input_ids=input_ids,
                                                attention_mask=torch.tensor(attention_mask, device=self.device),
                                                token_type_ids=torch.tensor(token_type_ids, device=self.device))
            sentiment_score3 = softmax(Bert_outputs[0], dim=1)[:, 1] - 0.5
            # print(f'sentiment_score3.shape:{new_sentiment_score.shape}') # sentiment_score3.shape:torch.Size([64])
            ...
            def _RL_loss(R, P_wt):
                ...
                '''
                loss = -((R.view(-1, 1)) * torch.log(P_wt.float())).mean(dim=1)
                return loss

            # RL_loss=_RL_loss((new_sentiment_score.detach()-sentiment_score.detach()-baseline_reward.detach()),P_wt)
            RL_loss=_RL_loss((sentiment_score3.detach()-sentiment_score1.detach()),P_wt)
            return RL_loss.mean(),mle_loss,sentiment_score2.detach()
