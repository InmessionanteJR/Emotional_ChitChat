...
from model import Transformer, condition_generator, combiner, estimator
...
class ModelWrapper(nn.Module):
    ...
    def forward(self,  # type: ignore
                batch: tx.data.Batch,condition_generator,posterior_mean_estimator,posterior_variance_estimator,key_value_combiner,K,V,sentiment_classifier,tokenizer) -> Dict[str, torch.Tensor]:
    ...
    def predict(self, batch: tx.data.Batch,condition_generator,posterior_mean_estimator,posterior_variance_estimator,key_value_combiner,K,V) -> Dict[str, torch.Tensor]:
...
def main() -> None:
    ...
    if args.run_mode == "train":
        ...
        def _save_epoch_per_1k_steps(epoch,step):
            ...
            checkpoint_name = f"System_checkpoint{epoch}_{step}.pt"
            print(f"saving ... {checkpoint_name}")
            torch.save(System.state_dict(), output_dir / checkpoint_name)
        ...
        def _train_epoch(epoch,K,V):
            ...
            for batch in data_iterator:
                return_dict = System(batch,System_condition_generator,posterior_mean_estimator,posterior_variance_estimator,key_value_combiner,K,V,sentiment_classifier,tokenizer)

                RL_loss,mle_loss,KL_loss,sentiment_scores,key,value= return_dict['loss']
                mixed_loss=RL_loss*config_model.alpha+mle_loss*(1-config_model.alpha)+KL_loss*config_model.gamma
                # mixed_loss=RL_loss
                optim.zero_grad()
                mixed_loss.backward()
        ...
        model_loaded = torch.load(args.output_dir+f'/System_checkpoint{args.epoch_id}_{args.step_id}.pt')
        model.load_state_dict(model_loaded)
        model.to(device)

        # Create condition generator
        System_condition_generator=condition_generator(config_model.hidden_dim).to(device)
        System_condition_generator.load_state_dict(torch.load(args.output_dir+f'/System_condition_generator_checkpoint{args.epoch_id}_{args.step_id}.pt'))

        # posterior mean and variance estimator
        posterior_mean_estimator = nn.Linear(config_model.hidden_dim, config_model.c_hidden_dim).to(device)
        posterior_mean_estimator.load_state_dict(torch.load(args.output_dir+f'/Posterior_mean_estimator{args.epoch_id}_{args.step_id}.pt'))
        posterior_variance_estimator = nn.Linear(config_model.hidden_dim, config_model.c_hidden_dim).to(device)
        posterior_variance_estimator.load_state_dict(torch.load(args.output_dir + f'/Posterior_variance_estimator{args.epoch_id}_{args.step_id}.pt'))
        ...
        with torch.no_grad():
            for batch in data_iterator:
                return_dict = model.predict(batch,System_condition_generator,posterior_mean_estimator,posterior_variance_estimator,key_value_combiner,K,V)
