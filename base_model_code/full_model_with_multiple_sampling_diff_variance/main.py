...
class ModelWrapper(nn.Module):
    ...
    def forward(self,  # type: ignore
                batch: tx.data.Batch,condition_generator,prior_variance_estimator,posterior_mean_estimator,posterior_variance_estimator,key_value_combiner,K,V,sentiment_classifier,tokenizer) -> Dict[str, torch.Tensor]:
        ...
        loss = self.model(encoder_input=src_text_ids,
                          ...
                          prior_variance_estimator=prior_variance_estimator,
                          posterior_mean_estimator=posterior_mean_estimator,
                          posterior_variance_estimator=posterior_variance_estimator,
    ...
    def predict(self, batch: tx.data.Batch,condition_generator,prior_variance_estimator,posterior_mean_estimator,posterior_variance_estimator,key_value_combiner,K,V) -> Dict[str, torch.Tensor]:
...
def main() -> None:
    ...
    if args.run_mode == "train":
        ...
        # posterior mean and variance estimator
        prior_variance_estimator=nn.Linear(config_model.hidden_dim, 10).to(device)
        posterior_mean_estimator=nn.Linear(config_model.hidden_dim, config_model.c_hidden_dim).to(device)
        posterior_variance_estimator=nn.Linear(config_model.hidden_dim, config_model.c_hidden_dim).to(device)
        # posterior_mean_estimator = estimator().to(device)
        # posterior_variance_estimator = estimator().to(device)
        ...
        # kmeans_model
        kmean_model = KMeans(n_clusters=1000, max_iter=30, n_jobs=-1)
        ...
        optim = torch.optim.Adam([
                # {'params': System.parameters()}, # freeze transformer
                {'params': prior_variance_estimator.parameters()},
                {'params': posterior_mean_estimator.parameters()},
                {'params': posterior_variance_estimator.parameters()},
        ...
        # resume checkpoints
        # System_condition_generator.load_state_dict(torch.load(args.output_dir+f'/System_condition_generator_checkpoint{5}.pt'))
        # key_value_combiner.load_state_dict(torch.load(args.output_dir + f'/Combiner_checkpoint{5}.pt'))
        # posterior_mean_estimator.load_state_dict(torch.load(args.output_dir + f'/Posterior_mean_estimator{5}.pt'))
        # posterior_variance_estimator.load_state_dict(torch.load(args.output_dir + f'/Posterior_variance_estimator{5}.pt'))
        # System.load_state_dict(torch.load(args.output_dir+f'/System_checkpoint{5}.pt'))
        # kmean_model = torch.load(args.output_dir + f'/kmean_model_{5}')
        # K = torch.load(args.output_dir+f'/cluster_key_centers_{5}').float().to(device)  # cluster centers for keys
        # V = torch.load(args.output_dir+f'/cluster_value_centers_{5}').float().to(device)  # weighted representations for each key
        # optim.load_state_dict(torch.load(args.output_dir + f'/optimizer{5}.pt'))
        # scheduler.load_state_dict(torch.load(args.output_dir + f'/scheduler{5}.pt'))
        ...
        def _save_epoch(epoch):
            ...
            checkpoint_name = f"Prior_variance_estimator{epoch}.pt"
            print(f"saving ... {checkpoint_name}")
            torch.save(prior_variance_estimator.state_dict(), output_dir / checkpoint_name)

            checkpoint_name = f"Posterior_mean_estimator{epoch}.pt"
            print(f"saving ... {checkpoint_name}")
            torch.save(posterior_mean_estimator.state_dict(), output_dir / checkpoint_name)

            checkpoint_name = f"Posterior_variance_estimator{epoch}.pt"
            print(f"saving ... {checkpoint_name}")
            torch.save(posterior_variance_estimator.state_dict(), output_dir / checkpoint_name)
        ...
        def _save_epoch_per_1k_steps(epoch,step):
            ...
            checkpoint_name = f"Posterior_mean_estimator{epoch}_{step}.pt"
            print(f"saving ... {checkpoint_name}")
            torch.save(posterior_mean_estimator.state_dict(), output_dir / checkpoint_name)

            checkpoint_name = f"Posterior_variance_estimator{epoch}_{step}.pt"
            print(f"saving ... {checkpoint_name}")
            torch.save(posterior_variance_estimator.state_dict(), output_dir / checkpoint_name)
        ...
        def _save_loss():
            ...
            name3="KL_loss"
            name4="mixed_loss"
            name5="avg_sentiment_scores"
            torch.save(RL_LOSS, output_dir / name1)
            torch.save(MLE_LOSS, output_dir / name2)
            torch.save(KL_LOSS, output_dir / name3)
            torch.save(MIXED_LOSS, output_dir / name4)
            torch.save(SENTIMENT_SCORES, output_dir / name5)
        ...
        def _caculate_kmean_centers(kmean_model,epoch):
            ...
            torch.save(KEY, output_dir / f"KEY_{epoch}")
            torch.save(VALUE, output_dir / f"VALUE_{epoch}")

            start = time.time()
            print(f'KEY.shape:{KEY.shape}')
            print(f'VALUE.shape:{VALUE.shape}')
            kmeans=kmean_model.fit(KEY)
            ...
            for i in range(len(kmeans.labels_)):
                if kmeans.labels_[i] in value_in_clusters:
                    value_in_clusters[kmeans.labels_[i]] = torch.cat(
                        (value_in_clusters[kmeans.labels_[i]], VALUE[i].unsqueeze(dim=0)), dim=0)
                else:
                    value_in_clusters[kmeans.labels_[i]] = VALUE[i].unsqueeze(dim=0)
        ...
        def _train_epoch(epoch,K,V):
            print(f'hyper-parameters: mixed_loss={config_model.alpha}*mle_loss+(1-{config_model.alpha})*RL_loss+{config_model.gamma}*KL_loss')
            ...
            for batch in data_iterator:
                return_dict = System(batch,System_condition_generator,prior_variance_estimator,posterior_mean_estimator,posterior_variance_estimator,key_value_combiner,K,V,sentiment_classifier,tokenizer)

                RL_loss,mle_loss,KL_loss,sentiment_scores,key,value= return_dict['loss']
                mixed_loss=(RL_loss*config_model.alpha+mle_loss*(1-config_model.alpha)+KL_loss*config_model.gamma)/config_data.accumulation_steps
                # mixed_loss=RL_loss
                ...
                KL_loss_stats.append(KL_loss.item())
                mixed_loss_stats.append(mixed_loss.item())
                sentiment_scores_stats.append(sentiment_scores.mean())
                # print(f'len(sentiment_scores_stats):{len(sentiment_scores_stats)}')
                # print(f'sentiment_scores.shape:{sentiment_scores.shape}')
                ...
                if KEY==None:
                    ...
                    KEY=torch.cat((KEY,key),0)
                    VALUE=torch.cat((VALUE,value),0)
                    # print(f'VALUE.shape:{VALUE.shape}')
                ...
                if step % config_data.display == 0:
                    ...
                    avr_KL_loss = sum(KL_loss_stats) / len(KL_loss_stats)
                    ...
                    KL_LOSS.append(avr_RL_loss)
                    MIXED_LOSS.append(avr_mixed_loss)
                    PPL.append(ppl)
                    SENTIMENT_SCORES.append(avg_sentiment_scores)

                    print(f"epoch={epoch}, step={step}/{num_steps}, mle_loss={avr_mle_loss:.4f}, ppl={ppl:.4f}, RL_loss={avr_RL_loss:.4f}, KL_loss={avr_KL_loss:.4f}, mixed_loss={avr_mixed_loss:.4f}, avg_sentiment_scores:{avg_sentiment_scores},lr={scheduler.get_last_lr()[0]}")
                ...
                if step % 1000 == 0:
                    ...
                    print('Empty cache!')
                    del return_dict,RL_loss,KL_loss,mle_loss,mixed_loss
        ...
        # posterior mean and variance estimator
        prior_variance_estimator = nn.Linear(config_model.hidden_dim, 10).to(device)
        prior_variance_estimator.load_state_dict(torch.load(args.output_dir + f'/Prior_variance_estimator{args.epoch_id}.pt'))

        posterior_mean_estimator = nn.Linear(config_model.hidden_dim, config_model.c_hidden_dim).to(device)
        posterior_mean_estimator.load_state_dict(torch.load(args.output_dir+f'/Posterior_mean_estimator{args.epoch_id}.pt'))
        posterior_variance_estimator = nn.Linear(config_model.hidden_dim, config_model.c_hidden_dim).to(device)
        posterior_variance_estimator.load_state_dict(torch.load(args.output_dir + f'/Posterior_variance_estimator{args.epoch_id}.pt'))
        ...
        if args.epoch_id=='0':
            print(f'args.epoch_id:{args.epoch_id}')
            K=torch.load('outputs/extract_centers/cluster_key_centers').float().to(device) # cluster centers for keys
            V=torch.load('outputs/extract_centers/cluster_value_centers').float().to(device) # weighted representations for each key
        else:
            print(f'args.epoch_id:{args.epoch_id}')
            K = torch.load(args.output_dir + f'/cluster_key_centers_{str(int(args.epoch_id)-1)}').float().to(device)  # cluster centers for keys
            V = torch.load(args.output_dir + f'/cluster_value_centers_{str(int(args.epoch_id)-1)}').float().to(device)  # weighted representations for each key
        ...
        with torch.no_grad():
            for batch in data_iterator:
                return_dict = model.predict(batch,System_condition_generator,prior_variance_estimator,posterior_mean_estimator,posterior_variance_estimator,key_value_combiner,K,V)
