...
class Transformer(nn.Module):
    ...
    def forward(self,  # type: ignore
                ...
                prior_variance_estimator:torch.nn.Module,
        ...
        if merge_input is not None:
            ...
            # print(f'representation:{c}')
            # print(f'torch.norm(K.detach(),p=2):{torch.norm(K.detach(),p=2)}')
            # print(f'torch.norm(c,p=2):{torch.norm(c,p=2)}')
            similarity = torch.matmul(c, K.detach().t())/(torch.norm(c,p=2)*torch.norm(K.detach(),p=2))
            print(f'similarity:{similarity}')
            top_k_index=torch.topk(similarity,k=10,dim=2) # [1] means 'indices'

            value_with_memory = V.detach()[top_k_index[1][0]] # torch.Size([64, 10, 512])
            # print(f'value_with_memory.shape:{value_with_memory.shape}')    # torch.Size([64, 10, 512])
            print(f'top_k_index[1][0][:5]:{top_k_index[1][0][:5]}')
            # print(f'top_k_index[0][0].shape:{top_k_index[0][0].shape}') # top_k_index[0][0].shape:torch.Size([64, 10])
            # print(f'top_k_index[0][0]:{top_k_index[0][0]}')

            weight=softmax(100*torch.reciprocal(top_k_index[0][0].max(dim=1)[0].reshape(-1, 1).repeat(1, top_k_index[0][0].shape[1]))*top_k_index[0][0],dim=1) # with temperature：the largest number in each row
            # weight=softmax(50000*top_k_index[0][0],dim=1) # with temperature # torch.Size([64, 10])
            print(f'weight[0]:{weight[0]}')
            # print(f'weight:{weight}')
            # weighted_value_with_memory=torch.matmul(torch.unsqueeze(weight, dim=1),value_with_memory)


            prior_mean = value_with_memory # torch.Size([64, 10, 512])
            prior_log_variance_square = 5*torch.sigmoid(torch.unsqueeze(prior_variance_estimator(torch.squeeze(c,dim=0)), dim=2)).repeat(1, 1, 512)
            print(f'prior_log_variance_square[:2,:,0]:{prior_log_variance_square[:2,:,0]}')

            posterior_mean=torch.unsqueeze(torch.squeeze(posterior_mean_estimator(c),dim=0),dim=1).repeat(1,10,1)
            # print(f'posterior_mean.shape:{posterior_mean.shape}')
            posterior_log_variance_square=torch.unsqueeze(torch.squeeze(posterior_variance_estimator(c),dim=0),dim=1).repeat(1,10,1)
            # print(f'posterior_log_variance_square.shape:{posterior_log_variance_square.shape}') # torch.Size([1, 64, 512])

            sampled_c=torch.unsqueeze(posterior_mean[:,0,:]+posterior_log_variance_square[:,0,:]*torch.randn_like(posterior_log_variance_square[:,0,:]),dim=0) # reparameterization
            # c=key_value_combiner(c,sampled_c)

            # KL divergence between 2 normal distributions： https://stats.stackexchange.com/questions/7440/kl-divergence-between-two-univariate-gaussians
            KL_loss=(0.5*torch.reciprocal(torch.exp(prior_log_variance_square))*(torch.exp(posterior_log_variance_square)*(prior_log_variance_square-posterior_log_variance_square)+torch.exp(posterior_log_variance_square)-torch.exp(prior_log_variance_square)+(posterior_mean-prior_mean)*(posterior_mean-prior_mean))*torch.unsqueeze(weight,dim=2)).mean()  # https://spaces.ac.cn/archives/5253#%E6%9D%A1%E4%BB%B6VAE
            ...
            src_with_emotional=torch.cat((first_token,torch.transpose(sampled_c, 0, 1),rest_tokens),dim=1)
