...
class Transformer(nn.Module):
    ...
    def forward(self,  # type: ignore
        ...
        if decoder_input is not None and labels is not None: # train-mode (teacher-forcing training)
            ...
            vocab = tx.data.Vocab('data/dict400w',eos_token='[SEP]')
            ...
            sentiment_score = softmax(Bert_outputs[0], dim=1)[:, 1]-0.5
            ...
            # Pass_this_batch=False
            # if token_level_sentiment_score.shape[1]-P_wt.shape[1]!=2: # this is due to BertTokenization's subwords mechanism, eg: if 'away' is not in dict, then BertTokenizerFast('away')->['a','###way']
            #     Pass_this_batch=True
            #     return None,None,None,None,Pass_this_batch

            # for i in range(token_level_sentiment_score.shape[0]):
            #     print(pred_batch_sentence[i])
            #     print(token_level_sentiment_score[i][1:-1])
            #     print()
            # print('--------------------------------------------------------------------')
            ...
            def _RL_loss(R, P_wt):
                ...
                '''
                # loss = ((R.view(-1, 1) - Rt) * torch.log(P_wt.float())).mean(dim=1)
                # loss = -((R.view(-1, 1)) * torch.log(P_wt.float()+1e-9)).mean(dim=1)
                loss = -(R * torch.log(P_wt.float())).mean(dim=1)
                return loss
            ...
            # part_loss=attention_mask[:,1:-1] * torch.log(token_level_sentiment_score[:,1:-1]*token_level_sentiment_score[:,1:-1]+1e-9)
            # print(f'part_loss:{part_loss}')
            # minus=(sentiment_score.detach().view(-1, 1)-token_level_sentiment_score[:,1:-1]) * attention_mask[:,1:-1]
            # print(f'minus:{minus}')
            # TLSA_loss=(minus*minus).mean()
            return RL_loss.mean(),mle_loss,sentiment_score.detach()
